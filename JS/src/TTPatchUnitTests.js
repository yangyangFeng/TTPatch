 ...
 */
_import(
  "TTDFKitURLSession,NSURLRequest,NSURL,NSString,TTPatchHotRefrshTool,UIDevice,UIView,UILabel,UIColor,UIFont,UIScreen,UIImageView,UIImage,UITapGestureRecognizer,UIButton,TTPlaygroundModel"
);
...
defineClass(
  "TTPatchUnitTests",
  {
    ...
    blockAddSignatureCase: function () {
      ...
      self.testCall0_(
        block("", function () {
          Utils.log_info(
            "[unit-test] --------JS传入OC方法,接受到回调--------- 无参数,无返回值"
          );
        })
      );
      self.testCall1_(
        block("void,id,int", function (arg1, arg2) {
          ...
          Utils.log_info(
            "[unit-test] --------JS传入OC方法,接受到回调--------- 有参数,无返回值  " +
              dic.name +
              "-" +
              arg2
          );
      ...
      self.testCall2_(
        block("id,id", function (arg) {
          Utils.log_info(
            "[unit-test] --------JS传入OC方法,接受到回调--------- 有参数,有返回值:string  " +
              arg
          );
      ...
      self.testCall3_(
        block("id,void", function () {
          Utils.log_info(
            "[unit-test] --------JS传入OC方法,接受到回调--------- 无参数,有返回值:string  "
          );
          return "这是有返回值的哦";
        })
      );
    },
    funcWithBlockParams_param2_: dynamic(
      "void,id,?",
    ...
    funcWithBlockParams_paramInt2_: dynamic(
      "void,id,?",
      function (arg1, callback) {
    ...
    funcWithParams_param2_: dynamic("void,id,id", function (param1, param2) {
      ...
      var arg2 = param2["key"];
      Utils.log_info(
        "[unit-test] [3]funcWithParams_param2_:" + param1 + "," + arg2
      );
    }),
  ...
  },
  {
    ...
    testAction_: function (str) {
      Utils.log_info("[unit-test] 静态方法调用成功-> " + str);
    },
  }
);
