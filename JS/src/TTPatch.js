...
MessageQueue.replaceMethod = function (
  className,
  superClassName,
  key,
  isInstanceMethod,
  ...
  return MessageQueue_oc_replaceMethod(
    className,
    superClassName,
    key,
    isInstanceMethod,
...
MessageQueue.replaceDynamicMethod = function (
  className,
  superClassName,
  key,
  isInstanceMethod,
  ...
  return MessageQueue_oc_replaceDynamicMethod(
    className,
    superClassName,
    key,
    isInstanceMethod,
...
MessageQueue.registerProperty = function (
  className,
  superClassName,
  propertys
) {
...
class Class_obj extends MetaObject {
  constructor(
    ...
    this.__cls = classMethods
      ? new Class_obj(className, superClassName, classMethods, false, null)
      : null;
  }
...
class Block extends JSObject {
  ...
  getBlock(arg) {
    ...
    for (let i = indexOffset; i < arguments.length; i++) {
      if (!params) params = new Array();
      params.push(
        isJsFunc ? pv_toJSObject(arguments[i]) : pv_toOcObject(arguments[i])
      );
    }
...
(function () {
  ...
  Object.prototype._c = function (msg) {
    ...
    if (jsMethod_IMP && !ttdfkit__isSuperInvoke) {
      ...
      result = MessageQueue.call(
        this.__isa,
        ttdfkit__isSuperInvoke,
        isInstance,
        msg,
        params
      );
  ...
  function pv_addPropertys(cls) {
    MessageQueue.registerProperty(
      cls.__className,
      cls.__superClassName,
      cls.__property_list
    );
  }
  ...
  function pv_registMethods(cls) {
    ...
    for (const key in cls.__methodList) {
      ...
      let signature = cls.__dynamicSignatureList
        ? cls.__dynamicSignatureList[key]
        : "";
      signature = signature ? signature : "";

      MessageQueue.replaceDynamicMethod(
        cls.__className,
        cls.__superClassName,
        key,
        isInstanceMethod,
  ...
  pv_toOcObject = function (arg) {
    ...
    let obj;
    if (arg instanceof Block) {
      arg.__blockHelper = MessageQueue.MessageQueue_oc_genBlock(
        arg.__signature,
        arg.getBlock.bind(arg)
      );
